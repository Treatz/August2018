    ...
    def func(self):
        ...
        if(caller.db.present or caller.location.db.freeze == 0):
            ...
            if not obj.access(caller, 'get'):
                ...
                return
            if((obj.db.alive == caller.db.alive) or (caller.db.touch == 1)):
...
class CmdSay(COMMAND_DEFAULT_CLASS):
    ...
    def func(self):
        ...
        if(caller.db.present):
            checktest = self.caller.search("Corey", global_search=True)
            ...
            for item in self.caller.location.contents:
                 if (item is not self.caller and item.db.alive == 1 and self.caller.db.alive == 1 and item.db.conscious == 1 and self.caller.db.conscious ==1):
                     item.msg(emit_string)
                     if( item.db.npc == 1):
                         [obj.msg('%s says to %s: %s' % (caller.name, item.name, self.args)) for obj in Character.objects.all() if obj.locks.check_lockstring(obj, "dummy:perm(builder)")]
