...
class OverLook(default_cmds.CmdLook):
    key = "look"
    def func(self):
        if not self.caller.location.db.freeze or (self.caller.db.present):
            if((self.caller.db.conscious == 1 and self.caller.db.alive == 1) or (self.caller.db.conscious == 1 and self.caller.db.sight == 1)):
                super(OverLook, self).func()
                healthbar = "|/|X|[wHealth:"
                total = self.caller.db.lethal + self.caller.db.bashing

                diff = 8 - self.caller.db.lethal

                if self.caller.db.bashing > diff:
                    self.caller.db.lethal += self.caller.db.bashing - diff
                    self.caller.db.bashing -= self.caller.db.bashing - diff
                for i in range(0,8):
                    if i < self.caller.db.lethal:
                        healthbar += " X"
                    elif i < total:
                        healthbar += " /"
                    else:
            ...
            self.caller.msg("Time has stopped.")
...
class OverSay(general.CmdSay):
    ...
    def func(self):
        if not self.caller.location.db.freeze or (self.caller.db.present):
            if((self.caller.db.conscious == 1 and self.caller.db.alive == 1) or (self.caller.db.conscious == 1 and self.caller.db.touch == 1)):
                super(OverSay, self).func()
                if(self.caller.db.spy != None and self.caller.location != self.caller.db.spy.location):
                    self.caller.db.spy.msg('%s says, "%s|n"' % (self.caller.name, self.args))
            else:
                self.caller.msg("You can't do that while unconscious")
...
class OverGet(general.CmdGet):
    ...
    def func(self):
        if not self.caller.location.db.freeze or (self.caller.db.present):
            if((self.caller.db.conscious == 1 and self.caller.db.alive == 1) or (self.caller.db.conscious == 1 and self.caller.db.touch == 1)):
                var1 = super(OverGet, self).func()
                self.caller.msg(var1)
            else:
                self.caller.msg("You can't do that while unconscious")
